...
export default class EditEstrela extends Component {
  ...
  componentDidMount() {
    axios.get('http://localhost:5000/estrelas/'+this.props.match.params.id)
      .then(response => {
  ...
  onSubmit(e) {
    ...
    axios.post('http://localhost:5000/estrelas/update/' + this.props.match.params.id, estrela)
            ...
            <MDBCard>
              <MDBCardBody>
                <form onSubmit={this.onSubmit}>
                  <h3 className="h4 text-center py-4">Editar Estrela {this.state.nome}</h3>
